...
// Search YouTube Videos using play-dl (FAST)
async function searchGoogleVideos(query) {
  try {
    console.log(`[SEARCH] Searching YouTube for: ${query}`);
    
    // Use play-dl search - load only 5 initially for EXTREME speed
    ...
    const videos = results.map(v => ({
      ...
      link: v.url || `https://www.youtube.com/watch?v=${v.id}`,
      ...
      views: v.views ? `${v.views.toLocaleString()} views` : 'N/A',
...
app.get('/search', async (req, res) => {
  ...
  // Enable aggressive caching (cache for 1 hour)
  ...
  try {
    ...
    // Pagination - load 5 per page for extreme speed
    const perPage = 5;
...
// SS DOWNLOADER Integration - Get video info using yt-dlp
app.get('/ss-downloader/get-info', async (req, res) => {
  const url = req.query.url;
  if (!url) return res.status(400).json({ error: 'No URL provided' });

  try {
    console.log(`[SS-DL] Getting info for: ${url}`);
    ...
    ytdlp.on('close', (code) => {
      if (code !== 0) {
        console.error('[SS-DL] yt-dlp error:', error);
        return res.status(500).json({ error: 'Video not found or unavailable' });
      }
      ...
      try {
        ...
        console.error('[SS-DL] Parse error:', e.message);
        res.status(500).json({ error: 'Failed to parse video info' });
      }
    });
  } catch (error) {
    console.error('[SS-DL] Error:', error.message);
    res.status(500).json({ error: 'Error processing URL' });
  }
});

// SS DOWNLOADER Integration - Download MP3
app.get('/ss-downloader/download-mp3', async (req, res) => {
  const url = req.query.url;
  const title = req.query.title || 'audio';
  if (!url) return res.status(400).json({ error: 'No URL provided' });

  try {
    console.log(`[SS-DL] Converting: ${url}`);
    
    // Clean filename (remove special characters)
    const cleanTitle = title.replace(/[^a-zA-Z0-9\s-]/g, '').replace(/\s+/g, ' ').trim().slice(0, 100);
    ...
    ytdlp.stderr.on('data', (data) => {
      console.error('[SS-DL] yt-dlp error:', data.toString());
    });

    ytdlp.on('error', (err) => {
      console.error('[SS-DL] Process error:', err.message);
      if (!res.headersSent) {
...
// Download MP3 from YouTube with real filename
app.get('/download-mp3', async (req, res) => {
  ...
  try {
    console.log(`[MP3] Converting: ${url}`);
    ...
    ytdlp.stderr.on('data', (data) => {
      console.error('[MP3] yt-dlp error:', data.toString());
    });

    ytdlp.on('error', (err) => {
      console.error('[MP3] Process error:', err.message);
      if (!res.headersSent) {
